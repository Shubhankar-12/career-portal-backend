...
import { UpdateCompanyDtoConverter } from "./dto";
import { UpdateCompanyUseCase } from "./usecase";
import { UpdateCompanyRequest } from "./request";
import { UpdateCompanyValidator } from "./validator";
...
export class UpdateCompanyController {
  ...
  constructor(updateCompany: UpdateCompanyUseCase) {
  ...
  async handle(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Validate Request
      const validator = new UpdateCompanyValidator(requestData);
      ...
      // Convert DTO
      const dtoObject = new UpdateCompanyDtoConverter(requestData);
      const token =
