...
import { GetJobByIdDtoConverter } from "./dto";
import { GetJobByIdUseCase } from "./usecase";
import { GetJobByIdRequest } from "./request";
import { GetJobByIdValidator } from "./validator";
...
export class GetJobByIdController {
  ...
  constructor(getJobByIdUseCase: GetJobByIdUseCase) {
    this.getJobByIdUseCase = getJobByIdUseCase;
  }

  async handle(req: Request, res: Response): Promise<void> {
    try {
      const requestData = req.query as unknown as GetJobByIdRequest;

      // Validate Request
      const validator = new GetJobByIdValidator(requestData);
      ...
      // Convert DTO
      const dtoObject = new GetJobByIdDtoConverter(requestData);
  ...
  private handleError(res: Response, error: any): void {
    console.error("GetJobByIdController Error:", error);
